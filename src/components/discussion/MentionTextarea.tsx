...
export const MentionTextarea = React.forwardRef<HTMLTextAreaElement, MentionTextareaProps>(
  (
    ...
    // Expose internal textareaRef through forwardedRef
    ...
    // Sync mentioned IDs automatically whenever value, candidates, or mentionedMap change
    useEffect(() => {
      const validIds = new Set<string>();

      // 1. From candidate list (programmatic insertion or manual typing)
      candidates.forEach((c) => {
        if (value.includes(`@${c.fullName}`)) {
          validIds.add(c.id);
        }
      });

      // 2. From local map (when picked from autocomplete)
      mentionedMap.forEach((id, name) => {
        if (value.includes(`@${name}`)) {
          validIds.add(id);
        }
      });

      const newIdsArray = Array.from(validIds).sort();
      const prevSorted = [...prevIdsRef.current].sort();
      const isDifferent =
        newIdsArray.length !== prevSorted.length ||
        newIdsArray.some((id, idx) => id !== prevSorted[idx]);

      if (isDifferent) {
        prevIdsRef.current = newIdsArray;
        onMentionedUsersChange(newIdsArray);
      }
    }, [value, candidates, mentionedMap, onMentionedUsersChange]);
