...
import { QuestionPaperExportParams } from '@/types/question';
import {
  FileText,
  X,
  Download,
...
interface QuestionPaperExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  subjectName: string;
  gradeLevel?: string;
  ...
  onExport: (params: QuestionPaperExportParams, format: 'DOCX' | 'PDF') => Promise<void>;
}

export default function QuestionPaperExportModal({
  isOpen,
  onClose,
  subjectName,
  gradeLevel,
  ...
  const [currentSubjectName, setCurrentSubjectName] = useState<string>('Toán học');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const gradePart = gradeLevel && gradeLevel !== 'ALL' ? ` - ${gradeLevel.toUpperCase()}` : '';
      const catPart = categoryName && categoryName !== 'ALL' ? ` (${categoryName})` : '';
      setExamTitle(`BỘ CÂU HỎI ÔN TẬP MÔN ${(subjectName || 'TOÁN HỌC').toUpperCase()}${gradePart}${catPart}`);

      setAcademicYear('Năm học: 2024 - 2025');
      setCurrentSubjectName(subjectName || 'Toán học');
      setDurationMinutes(40);
      setInstructionNote('Khoanh vào trước câu trả lời đúng nhất.');
      setExportFormat('DOCX');
      setErrorMessage(null);
    }
  }, [isOpen, subjectName, gradeLevel, categoryName]);
  ...
  const handleApplyPreset = (preset: 'TIỂU_HỌC' | 'THCS' | 'THPT' | 'TRUNG_TÂM') => {
    const gradePart = gradeLevel && gradeLevel !== 'ALL' ? ` - ${gradeLevel.toUpperCase()}` : '';
    const catPart = categoryName && categoryName !== 'ALL' ? ` (${categoryName})` : '';
    const subUpper = (currentSubjectName || subjectName || 'TOÁN HỌC').toUpperCase();

    if (preset === 'TIỂU_HỌC') {
      setInstitutionName('Trường Tiểu học Đồng Tâm');
      setDepartmentName('PHÒNG GIÁO DỤC VÀ ĐÀO TẠO');
      setExamTitle(`BỘ CÂU HỎI ÔN TẬP MÔN ${subUpper}${gradePart}${catPart}`);
      setAcademicYear('Năm học: 2024 - 2025');
      setInstructionNote('Khoanh vào trước câu trả lời đúng nhất.');
    } else if (preset === 'THCS') {
      setInstitutionName('Trường THCS Chu Văn An');
      setDepartmentName('PHÒNG GIÁO DỤC VÀ ĐÀO TẠO');
      setExamTitle(`BỘ CÂU HỎI ÔN TẬP HỌC KỲ MÔN ${subUpper}${gradePart}${catPart}`);
      setAcademicYear('Năm học: 2024 - 2025');
      ...
      setExamTitle(`BỘ CÂU HỎI KHẢO SÁT CHẤT LƯỢNG MÔN ${subUpper}${gradePart}${catPart}`);
      setAcademicYear('Năm học: 2024 - 2025');
      ...
      setDepartmentName('');
      setExamTitle(`BỘ CÂU HỎI ĐÁNH GIÁ NĂNG LỰC MÔN ${subUpper}${gradePart}${catPart}`);
      setAcademicYear('Năm học: 2024 - 2025');
      setInstructionNote('Khoanh vào trước câu trả lời đúng nhất.');
    }
  };
      ...
      await onExport(
        {
          ...
          subjectName: currentSubjectName.trim(),
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 animate-in zoom-in-95 duration-200 overflow-hidden">
        ...
        <div className="p-5 sm:p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-indigo-100 text-indigo-700 flex items-center justify-center shadow-md shadow-indigo-500/20">
              <Printer className="w-6 h-6" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base sm:text-lg font-bold text-slate-900">
                  In & Tải Bộ Câu Hỏi Chuẩn Bộ GD&ĐT
                </h3>
                <span className="px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-800 text-[11px] font-extrabold">
                  {totalQuestions} câu
                </span>
              </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-5 overflow-y-auto flex-1 text-xs sm:text-sm">
          {/* Notice: No answer key included */}
          <div className="p-3.5 rounded-2xl bg-blue-50/80 border border-blue-200/80 flex items-start gap-2.5">
            <Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
            <div className="text-xs text-blue-900">
              <p className="font-bold">Đề ôn tập chuẩn không kèm đáp án</p>
              <p className="text-blue-700 mt-0.5 text-[11px] leading-relaxed">
                Tài liệu tải về gồm tiêu đề, bảng điểm, thông tin học sinh, câu hỏi trắc nghiệm & dòng kẻ chấm chấm làm bài tự luận. Tuyệt đối không chứa đáp án hay lời giải để học sinh và giáo viên sử dụng làm bài ôn luyện.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl outline-none font-semibold text-slate-900 transition-all text-xs sm:text-sm"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl outline-none text-slate-900 transition-all text-xs sm:text-sm"
              />
            </div>
            ...
            <div className="sm:col-span-2">
              <label className="text-[11px] font-bold text-slate-700 uppercase tracking-wider block mb-1">
                Tiêu đề Đề kiểm tra / Bộ câu hỏi <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="VD: BỘ CÂU HỎI ÔN TẬP MÔN TOÁN HỌC - LỚP 1"
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl outline-none font-bold text-slate-900 transition-all text-xs sm:text-sm"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setCurrentSubjectName(e.target.value)}
              ...
              >
                <div
                  className={
                    ...
                    (exportFormat === 'DOCX' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500')
                  }
                >
            ...
            >
              {isExporting ? (
                ...
                <>
                  ...
                  <span>Tải Câu Hỏi ({exportFormat === 'DOCX' ? '.docx' : '.pdf'})</span>
                </>
              )}
            </button>
          </div>
